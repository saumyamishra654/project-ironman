...
- **Sep 2 — added Copenhagen planks (adductor prehab) to Posterior day (Thu).** The adductors are the flagged weak/cooked area (sore from the first sprints + newly loaded by the cadence shift — see running history). Copenhagens are the gold-standard adductor strengthener. Guardrails: **start only once the current sprint soreness fully clears** (don't load an already-cooked/possibly-tweaked adductor), and **start SHORT** (bent-knee/short-lever regression — they cause real DOMS when novel; ego-lever = strain risk). Placed Thursday so adductors recover before the Sat long run + Mon sprint (both use adductors heavily). 2 sets to start.

- **Week 5 (Sep 7–13) — NO standalone sprint; the TT is the week's speed work.** 5K TT is Wed Sep 9. Week 5 has three leg-priorities (TT Wed, sprint, long run Sat) that can't all fit without cooking one — sprints cook legs ~2 days while novel (this ruined the Sep 2 TT). Resolution: **skip the standalone sprint in Week 5** (the max TT is plenty of speed stimulus), keeping both the Wed TT and Sat long run on fresh legs. Week 5 = Mon easy · Tue Anterior · Wed TT · Thu Posterior · Fri easy · Sat long run + Upper · Sun swim. **The 100m speed block shifts to start Week 6 (Mon Sep 14)** — full 4-week progression intact, just one week later. Acceleration session moved to Mon Sep 14.
- **Sep 2 — 5K TT rebuilt & rescheduled to Wed Sep 9.** After the Sep 2 abort (legs cooked, not cardio — was on 5:27/km pace @ HR 167→175, engine had headroom), target reframed: no fixed pace ceiling. New workout is **HR-governed** (km1 162–175 controlled → build to threshold → km5 182+ no ceiling), so sub-28 is on the table if legs are fresh, without blow-up risk. 28:30 is the floor/reference, not a committed pace. Watch id 1684929005.

- **Week 4 (Sep 1–2, deload + 5K TT) — lift swap to protect the TT.** 5K TT is Wed Sep 2 AM. To avoid quad fatigue 12h out (Anterior has Goblet/BB Squat + DB Step-Ups — leg DOMS/neuromuscular fatigue peaks 12–48h, right on TT morning), **Tue = Upper (upper-body only, TT-safe), Anterior moved to Thu/Fri (post-TT).** Keep Tue Upper light (deload, leave reps in the tank). Tue AM stack: shakeout run FIRST (strides fresh) → easy swim (doubles as flush). One-week exception; normal layout resumes Week 5.

- **Aug 30 — weekly swim/run RESTRUCTURE (athlete logistics).** Monday swim wasn't working; weekend swims are easier. New weekly layout: **Mon** sprint session (wk5+) · **Tue** Anterior · **Wed** swim (threshold) + speed run · **Thu** Posterior · **Fri** swim (endurance) + easy run · **Sat** long run (AM, for heat) + Upper (PM) · **Sun** swim (technique, recovery). Rationale: Monday swim → Sunday; long run moved to Sat (kept off the same day as a swim to avoid the Upper+swim muscle overlap — Upper pairs with the long run instead, legs vs upper body); sprint on Monday (freshest day, 2 days post-long-run) with easy run on Friday. 3 swims land Wed/Fri/Sun. NOTE: dashboard week-grid shows this steady state; **Week 4 is the deload+TT exception** (Mon easy, no sprint; 5K TT Wed Sep 2).
- **From Week 5 (Sep 7+) — NEW recurring Monday Short Sprint Session (athlete-requested).** Short neuromuscular sprint session on **Mondays** (swapped from Friday for fresher legs). Athlete wants to develop his top-end (genuine 200–400m sprint ability, underused by the plan) — "for no reason other than I want to," and to see what happens. Chose **hill/ramp sprints** (Hebbal is flat → parking-garage ramp / flyover ramp / 6–8% treadmill; flat flying sprints or gym sled as fallback) because short hill sprints build patellar-tendon stiffness — protective given his patellar history, not threatening. Session: 2.5k easy WU → **6×12s ~90–95% hill sprints, full walk-down recovery (~2.5 min)** → 1.5k CD (~5k total, low systemic cost). Progress by adding reps (6→8→10), not intensity. **Knee-gated:** any ache below the kneecap = stop immediately (patellar tendinopathy watch). First session scheduled Mon Sep 7 (watch id 1681247829).

- **Week 9 (Oct 5–11) — TENTATIVE: tempo 10k race replaces the Sunday long run.** Possible local 10k race Sun Oct 11 (block week 9). Plan: run it as a **tempo** (not chill) — it becomes the week's quality piece. The 19k long run is **dropped** (not made up midweek — weekend long runs are hard to move to weekdays, and one lighter week with a race costs nothing off an established 16–18k long-run base). Week 9 dips to ~30k; accepted. Wed intervals optional (drop if not wanting 2 hard days). **Watch item:** Week 10 (Oct 18) is the 21k debut, now following two lighter weeks (W8 deload + W9 race) — run it genuinely easy, no comp segments. Not yet firm; confirm the race before building.
- **Week 3 (Aug 24–30) — knee scrape (from Aug 26 speed-run fall):** skipped **Fri Aug 28 recovery 5k** and the **S3 threshold swim** to protect the healing scrape. Everything else this week done; weekend as planned (18k long run + S2 endurance swim). Surfaced on dashboard notice.
...
- **Aug 16:** Swim moves to **3 sessions/week from Week 3** (wk1-2 stay at 2). Three fixed skeletons scaled by main-set reps week to week: **S1 technique** (Mon AM, 2→4×200), **S2 endurance** (Fri AM, 1→3×800 continuous), **S3 threshold+speed** (Wed AM, 4→8×100, deloads go easy/no-sprint). Weekly ramp 6.1k (wk3) → 8.4k (wk10-11 peak), deloads at 4/8/12. Placement is polarized: Wed = one hard day (S3 swim + PM speed run); Mon = easy valve (S1 swim + recovery run); Fri = S2 aerobic. Adopted from a saved 3-swim template, rep-scaled to ramp volume rather than jumping. Caveat: skeletons floor ~5.7k so deloads deload by intensity (drop threshold) more than volume; watch paddle load vs Thu Posterior pulls.
- **Aug 16:** **Upper lift moved Fri → Sat from Week 2 onward** (Week 1 stays Fri, historical). Week 2 keeps 2 swims but Upper on Sat; Week 3+ adds the 3rd swim on top. Reason: unloads the Fri swim+Upper shoulder-stack and spaces lifts evenly (Tue Anterior / Thu Posterior / Sat Upper). Cost: loses the standalone Sat rest day — Monday becomes the recovery valve (keep S1 swim + recovery run genuinely easy) and take ad-hoc full rest when readiness/RHR/HRV dip. Sat Upper stays the lighter rep-focused bench (never heavy singles) so Sunday legs are fresh.
...
- **Aug 22 (week 2) — CSS TEST DONE. CSS = 2:18/100m (0.722 m/s).** First real swim benchmark. 400 TT 8:35 (2:08.8/100m, HR 173); 192 TT 3:47 (1:58.3/100m, HR 171, max 190); full recovery between (HR to 140), so TTs uncontaminated. Test valid — breaststroke was confined to WU/first-336/CD, never the TTs. **Swim training paces now: speed ~2:14, threshold ~2:18, endurance ~2:26 per 100m** (per 16 m length: ~21.5 / ~22 / ~23.5 s). Apply to S2 (~2:26) / S3 (~2:18) / speed reps (just under CSS). Interpretation: large 11 s/100m spread 400→192 = fatigue-resistance is the developing quality (top-end speed already there); expect CSS to drop as aerobic base builds. Feedback logged: keep 3×336 + aerobic filler freestyle so paces mean something.
  - _(Was: Aug 20 — CSS test scheduled Sat Aug 22 as 400 + 192 TT, 192 not 200 so both are whole 16 m lengths; CSS = 100/((400−192)/(t400−t192)). Now executed.)_
- **Aug 19–20 — acute load spike, watch recovery.** Aug 19 speed run was run on no sleep (slept upright on a train) → HR redlined to 188 avg / 196 max (VO2max TE 4.2) at 5:55/km — invalid as a fitness read, discard its Garmin execution score. Followed <24 h later by the Aug 20 swim. System is stacking: keep the Aug 20 Posterior lift light or push it, tomorrow genuinely easy. Scheduling-level only — plan stays locked.
