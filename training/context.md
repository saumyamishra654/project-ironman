...
- **Aug 20 (week 2) — CSS test scheduled Sat Aug 22.** Week 2 plan calls for the maximal TT to set CSS pace; the Aug 20 AM swim was controlled 272 m endurance reps, not the test. Now built + scheduled to Garmin for Sat Aug 22 (+ a split 3×336 aerobic km after) as **400 + 192 TT** (192 not 200 so both reps are whole lengths in the 16 m pool; CSS = 100/((400−192)/(t400−t192)) m per 100 m, computed from actual distances). Full recovery (lap-button) between the two efforts. Do it fresh — if Sat legs/arms are cooked from the week's load, push the TT rather than test depleted.
